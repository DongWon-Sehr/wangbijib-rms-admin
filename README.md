...
2. **실시간 동적 알림 (GNB Bell UI):**
   - 별도 DB 구축 없이 프론트엔드가 실시간 계산하여 알려주는 미결 상태 알림.
    - **`[예약]`** (노란 뱃지): 미래 방문건 중 확정 대기 중인 예약 (`확정 대기 중`).
    - **`[예약금]`** (주황 뱃지): 미래 방문건 중 예약금 입금 대기 중인 예약 (`입금 대기 중`).
    - **`[임박]`** (빨간 뱃지): 현재 시각 기준 **6시간 이내** 방문이 임박한 예약 (`방문 임박`).
    - **`[예약금]`** (파란 뱃지): 방문 예정 시각이 지났으나 예약금이 완료(확정) 상태인 예약 (`환불 필요`).
